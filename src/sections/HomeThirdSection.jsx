...
function TwistedText({ text }) {
  ...
  useEffect(() => {
    ...
    return () => { rtRef.current && rtRef.current.dispose() }
  }, [])

  // Add text mesh to render target scene
  useEffect(() => {
    if (textRef.current && rtSceneRef.current && !rtSceneRef.current.children.includes(textRef.current)) {
...
const HomeThirdSection = () => {
  return (
    <div className='bg-black h-screen w-full relative '>
      <Canvas className='w-full h-full absolute top-0 left-0 z-10' camera={{ position: [0, 0, 80], fov: 45 }}>
        <ambientLight intensity={0.5} />
        <pointLight position={[10, 10, 10]} intensity={1} />
        <TwistedText text="ALPHA" />
      </Canvas>
    </div>
  )
}
